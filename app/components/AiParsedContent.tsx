...
export default function YourParserComponent() {
  ...
  const [tailoredPoints, setTailoredPoints] = useState<Record<string, string[]> | string[] | null>(null);
  ...
  const handleTailorResume = async () => {
    ...
    try {
      const response = await fetch('/api/tailor', {
        ...
        body: JSON.stringify({ resumeData: parsedResumeData, jdData: parsedJdData }),
      });
      ...
      if (data?.tailored_resume && typeof data.tailored_resume === 'object' && !Array.isArray(data.tailored_resume)) {
    ...
    <div className="w-full max-w-6xl mx-auto p-4 md:p-8">
      ...
      <div className="mb-8 p-6 bg-blue-50 rounded-lg border">
        ...
        <input
          type="file"
          accept=".pdf"
          onChange={(e) => e.target.files?.[0] && handleResumeParse(e.target.files[0])}
          className="mb-4"
        />
      ...
      <div className="mb-8 p-6 bg-green-50 rounded-lg border">
        ...
        {parsedJdData && (
          <p className="text-green-600 mt-2">✅ Job description parsed successfully!</p>
        )}
      </div>
      ...
      <div className="my-8 text-center">
        ...
        >
          {isTailoring ? '🤖 Tailoring with AI...' : '✨ Tailor My Resume with AI'}
        </button>
      </div>
      ...
      {tailoredPoints && Array.isArray(tailoredPoints) && (
        <div className="mt-8 p-6 bg-white rounded-lg shadow-md border border-purple-200">
          <h2 className="text-2xl font-bold text-purple-700 mb-4">🎯 AI-Tailored Experience Section</h2>
      ...
      {tailoredPoints && !Array.isArray(tailoredPoints) && (
        <div className="mt-8 p-6 bg-white rounded-lg shadow-md border border-purple-200">
          <h2 className="text-2xl font-bold text-purple-700 mb-4">🎯 AI-Tailored Resume</h2>
          {Object.entries(tailoredPoints as Record<string, string[]>).map(([section, items]) => (
